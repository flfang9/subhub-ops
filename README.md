...
|---|---|---|
| `monday-checkin` | Mondays 11:00 UTC (7am EDT target) | **live**. Posts `checkin-message.md` to #building-in-public as the SubHub bot. Cron is set 2h early on purpose: GitHub's schedule queue fired 2-3h late on Jul 27 + Aug 3, so 11:00 UTC lands ~9am ET in practice. If it ever posts too early, bump back toward 12:00. Also @-mentions up to 5 `App Leaders` who joined the server 1 to 7 days ago, newest first, never the same person twice (it reads its own previous posts in the channel to know); mention copy is `checkin-new-leaders.md`, tune with `MENTION_LIMIT` / `MENTION_WINDOW_DAYS` / `MENTION_MIN_AGE_HOURS`. Preview the pick without posting: `DRY_RUN=true DISCORD_BOT_TOKEN=$(cat ~/.config/discord-audit/token) python3 scripts/post_checkin.py`. Unit tests: `python3 scripts/test_post_checkin.py` |
...
Edit the message copy by editing the `.md` files, next run picks it up. Copy style: Poke voice, lowercase for DMs, no em dashes (see `feedback-writing-style-no-em-dashes`). Secrets: `DISCORD_BOT_TOKEN` (the SubHub audit bot). Test the Monday post end-to-end via Actions → monday-checkin → Run workflow with `test_mode` (posts + deletes in ~2s, and notifies nobody: the test post renders the mentions as text but pings no one).
